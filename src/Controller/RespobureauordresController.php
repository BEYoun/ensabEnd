    ...
    public function envoyer()
    {
        ...
        if (!empty($resultdir)) {
            Email::configTransport('pers', [
                'host' => $resultdir['host'],
                'port' => $resultdir['port'],
                'username' => $resultdir['username'],
                'password' => $resultdir['password'],
                'className' => 'Smtp'
            ]);
            $transport = 'pers';
        }else{
...
//////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function downloadAcc($id = null){
        ...
        $this->set('personnalisation',$res2);        $CourrierArrivees=TableRegistry::get('CourrierArrivees');
        $courrierArrivee = $CourrierArrivees->get($id);
        $titre2=$connection->execute("SELECT id FROM courrier_arrivees where id=$id ")->fetchAll('assoc');
        $titre22=$titre2['0']['id'];
