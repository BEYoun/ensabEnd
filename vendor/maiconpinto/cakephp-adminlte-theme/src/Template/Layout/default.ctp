...
<!-- AdminLTE Skins. Choose a skin from the css/skins
    ...
    <?php if(!empty($personnalisation) and $personnalisation['theme']!=NULL){
            echo $this->Html->css('AdminLTE.skins/'.$personnalisation['theme'].'.min');
            }else{
...
<body class="hold-transition <?php
                                if(!empty($personnalisation) and $personnalisation['theme']!=NULL){
                                    echo $personnalisation['theme'];
                                }else{
                                    echo 'skin-blue';
                                }
    ...
    <div class="wrapper">
        <header class="main-header">
            <!-- Logo -->
            <a href="<?php echo $this->Url->build('/'); ?>" class="logo" style="background-color: <?php if(!empty($personnalisation) and $personnalisation['nav_color']!=NULL){
                                        echo $personnalisation['nav_color'];
                                    }
                          ?>;  ">
                <!-- mini logo for sidebar mini 50x50 pixels -->
                <span class="logo-mini"><?php echo "ADM" ?></span>
